...
locs= [1,2,3,4]
t= np.asarray(zip(locs[0], locs[1])) 
print (t)
...
class Exploration:
    ...
    def __init__(self, realMap="sample1.txt", timeLimit=None, calibrateLim=6, sim=True):
        """
        ...
        # removed calibreLim from original 

        self.timeLimit = timeLimit
        self.exploredArea = 0  # total explore area needs to be calculated to give perentage area explored 
...
# ------> find what virtual wall does 
    ...
    def validMove(self, inds):
        ...
        """
        for (r, c) in inds:
            # checks if r,c are in areana or not
            if not ((self.virtualWall[0] <= r < self.virtualWall[2]) and (self.virtualWall[1] <= c < self.virtualWall[3])):
                return False
    ...
    def frontFree(self):
        ...
        # check if all three blocks in horizontal in front are free or not
        # robot is 3x3 and need three spaces horizontally in front of it to move 
        # depending on the direction robot is facing, we move forward by checking the three blovks in front of the robot

        # returns number of moves the robot can make that are directly in front
    ...
    def moveStep(self, sensor_vals=None):
        ...
        # check if percentage of explored area is 100 or not
...
#------------------->might have to check if this statement is outsode the if statement/ while loop or not!!!!!
                    return neighbour
